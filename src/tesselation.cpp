...
std::vector<Quad> ExtractQuadsFromScene(const HydraGeomData& data) {
    const unsigned indicesCount = data.getIndicesNumber();

    std::vector<bool> used(indicesCount / 3, false);
    std::vector<Quad> result;
    for (unsigned i = 0; i < indicesCount; i += 3) {
        for (unsigned j = i + 3; j < indicesCount; j += 3) {
            if (used[j / 3]) {
                continue;
            }
            std::vector<unsigned> matchedIndices;
            std::vector<unsigned> secondTriangleMatchedIndices;
            ...
            const unsigned notMatchedIdx = 3 - matchedIndices[0] - matchedIndices[1];
            ...
            const unsigned forthIndex = 3 - secondTriangleMatchedIndices[0] - secondTriangleMatchedIndices[1];
    ...
    for (unsigned i = 0; i < indicesCount; ++i) {
        ...
        std::vector<unsigned>& counter = counters[planeIndex];
    ...
    for (unsigned i = 0; i < planes.size(); ++i) {
        ...
        std::vector<unsigned>& counter = counters[i];
        ...
        float maxDist = 0;
        for (unsigned j = 0; j < locVertices.size(); ++j) {
            for (unsigned h = j + 1; h < locVertices.size(); ++h) {
                maxDist = std::max(maxDist, distance(locVertices[j], locVertices[h]));
            }
        }
        for (unsigned j = 0; j < locVertices.size() - 2; ++j) {
            static const float eps = 1e-6;
        ...
        for (unsigned k = 0; k < locVertices.size(); k += 4) {
...
void SaveTessellation(const std::vector<Quad> &quads, const std::string &path) {
    ...
    for (auto& quad : quads) {
        ...
        indices.push_back(static_cast<unsigned>(points.size()) - 4);
        indices.push_back(static_cast<unsigned>(points.size()) - 3);
        indices.push_back(static_cast<unsigned>(points.size()) - 2);
        indices.push_back(static_cast<unsigned>(points.size()) - 4);
        indices.push_back(static_cast<unsigned>(points.size()) - 2);
        indices.push_back(static_cast<unsigned>(points.size()) - 1);
    }
    scene.setData(
